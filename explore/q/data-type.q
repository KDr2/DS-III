...
`type_tab insert (`p;     12; `timestamp; 0; `$"";      `$"");
...
`type_tab insert (`n;     16; `timespan;  0; `$"";      `$"-");
...
`type_tab insert (`$"*1"; 22; `enum;      4; `$"`s$.."; `$"`s$`b, where s:`a`b");
`type_tab insert (`$"*2"; 98; `table;     0; `$"";      `$"([]k:`a`b`c; d:1 2 3)");
`type_tab insert (`$"*3"; 99; `dict;      0; `$"";      `$"`a`b`c!1 2 3");

-1 "-----------------------------------------------------";

show (select from type_tab)

show flip{(neg x;upper .Q.t x;key'[x$\:()])}5h$where" "<>20#.Q.t
\\
